    ...
    public function setLomesData($damResource, $params)
{
    ...
    $params = $params->all();
    $tab_key = $params['_tab_key'];
    unset($params['_tab_key']);

    // Initialize the $lomes array if not already initialized
    $lomes = [];
    // Flatten the params array
    $flattenedParams = $this->flattenArray($params);

    // Store the flattened parameters under the tab key in the lomes array
    foreach ($flattenedParams as $key => $value) {
        if ($value !== null) {
            $lomes["lomes.$tab_key.$key"] = $value;
        }
    }
